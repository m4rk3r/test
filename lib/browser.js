...
function CBrowser(reqid, target_div, init_params) {
  ...
  function setup_browser() {
    ...
    var bcr = targetDivNode.getBoundingClientRect();
    var w = bcr.width;
    var h = bcr.height;
    ...
    req_params["audio"] = (0, _audio.getBestAudioType)();
  ...
  function init_browser() {
    ...
    fetch(init_url, options).then(function (res) {
      return res.json();
    }).then(function (data) {
  ...
  function handle_browser_response(data) {
    ...
    if (data.cmd_port && data.vnc_port) {
      ...
      if (init_params.audio) {
        ...
        if (data.audio) {
          window.audioPlugin = new _audio.WSAudio(data, init_params);
  ...
  function do_vnc() {
    ...
    var promise = new _promisePolyfill2.default(function (resolve, reject) {
      rfb = new _rfb2.default(target, webservice_url, { 'credentials': { 'password': vnc_pass } });
      //if (!rfbEventsBound) {
      credentialsRequired = function credentialsRequired() {
      ...
      securityFailure = function securityFailure() {
